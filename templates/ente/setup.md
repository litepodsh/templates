...
`ENTE_MINIO_ROOT_USER` / `ENTE_MINIO_ROOT_PASSWORD`) is exposed for browsing the
stored objects.
...
mounted `museum.yaml`. The S3 keys map to the fixed bucket name `b2-eu-cen`
(Ente requires that literal name even for local MinIO). If uploads fail with
storage errors, verify `ENTE_S3_B2_EU_CEN_*` reached the container
